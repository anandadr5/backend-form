...
function generateCommonEmailBody(
  ...
  const renderUrls = (urlField, linkTitle) => {
    if (!urlField || typeof urlField !== 'string' || urlField.trim() === '-') {
        // Mengembalikan list item dengan N/A jika data kosong
        return `<li><strong>${linkTitle}:</strong> N/A</li>`;
    }
    
    // Memecah string berdasarkan koma dan/atau spasi (sesuai output Apps Script yang sudah dibersihkan)
    const urlArray = urlField.split(/,\s*|\s+/).filter(url => url.startsWith('http'));

    if (urlArray.length === 0) {
        return `<li><strong>${linkTitle}:</strong> N/A</li>`;
    }

    // Menggabungkan semua URL, dipisahkan dengan <br/> untuk tampilan multi-baris
    const linksHtml = urlArray.map(url => 
        `<a href="${url}" target="_blank">${url}</a>`
    ).join('<br/>');

    return `<li><strong>${linkTitle}:</strong> ${linksHtml}</li>`;
  };
  ...
  if (isSupervisionSequence) {
      ...
      const spkUrlsHtml = renderUrls(data.spkUrl, "File SPK (Sipil & ME)");
      const rabUrlsHtml = renderUrls(data.rabUrl, "File RAB & Lampiran (Sipil & ME)");
  ...
  return `
    <div style="text-align: left; font-family: Arial, sans-serif; line-height: 1.6;">
      ...
      <ul>
        <li><strong>${dateLabel}:</strong> ${data.tanggal_mengawas}</li>
        ${additionalHtml}
        <li><strong>Kode Ulok:</strong> ${data.kode_ulok}</li>
        <li><strong>Nama Toko:</strong> ${data.nama_toko}</li>
        <li><strong>Cabang:</strong> ${data.cabang}</li>
