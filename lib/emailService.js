...
function generateCommonEmailBody(
  ...
  if (isKoorManagerOnly) {
      ...
      const names = [];
      if (data.manager_email && data.manager_nama) names.push(data.manager_nama);
      if (data.pic_building_support && data.pic_nama) names.push(data.pic_nama);
      if (data.koordinator_email && data.koordinator_nama) names.push(data.koordinator_nama);
      if (data.kontraktor_email && data.kontraktor_nama) names.push(data.kontraktor_nama); 
      
      generalSalamNameParts = names.filter(Boolean);
  } else {
  ...
  if (isKoorManagerOnly) {
      generalSalamNama = generalSalamNameParts.length > 0 
                         ? `Bapak/Ibu ${generalSalamNameParts.map(n => `<strong>${n}</strong>`).join(" dan ")}` 
      ...
      if (isSerahTerima && generalSalamNameParts.length === 1 && generalSalamNameParts[0] === data.kontraktor_nama) {
  ...
  const isContractorRejection = additionalInfo.isContractorRejection === true; 
  ...
  let linkUlangHtml = '';
  if (data.link_serah_terima_ulang && !isContractorRejection) { 
      linkUlangHtml = `
...
const emailTemplates = {
  ...
  "serah-terima": {
    ...
    getRecipients: (data) => {
      if (data.recipientsOverride && data.recipientsOverride.length > 0) {
        return data.recipientsOverride;
      }
    ...
    getHtmlBody: (data) =>
      generateCommonEmailBody(data, "serah-terima", "Serah Terima", "", true, {
        ...
        isContractorRejection: data.isContractorRejection
      }),
  },
