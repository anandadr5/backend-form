...
function generateCommonEmailBody(
  ...
  const lowerRecipientEmail = recipientEmail ? recipientEmail.toLowerCase() : null;

  if (lowerRecipientEmail) {
      if (data.pic_building_support && data.pic_building_support.toLowerCase() === lowerRecipientEmail) {
          recipientName = data.pic_nama;
      } else if (data.koordinator_email && data.koordinator_email.toLowerCase() === lowerRecipientEmail) {
          recipientName = data.koordinator_nama;
      } else if (data.manager_email && data.manager_email.toLowerCase() === lowerRecipientEmail) {
          recipientName = data.manager_nama;
      } else if (data.kontraktor_email && data.kontraktor_email.toLowerCase() === lowerRecipientEmail) {
          recipientName = data.kontraktor_nama; // <-- Tambahan untuk Kontraktor
      }
  }
  ...
  let generalSalamNama;
  if (recipientName) {
      generalSalamNama = `Bapak/Ibu <strong>${recipientName}</strong>`;
  } 
  
  else if (isKoorManagerOnly) {
      const koorManagerParts = [data.manager_nama, data.koordinator_nama].filter(Boolean);
      generalSalamNama = koorManagerParts.length > 0 
                         ? `Bapak/Ibu ${koorManagerParts.map(n => `<strong>${n}</strong>`).join(" atau ")}` 
                         : "Tim Building Support";
  } else if (isNonRukoInputPic) {
       generalSalamNama = data.pic_nama
                         ? `Bapak/Ibu <strong>${data.pic_nama}</strong>` 
                         : "Tim Building Support";
  } else {
      let allParts = [data.manager_nama, data.pic_nama, data.koordinator_nama];
      if (formType === "serah-terima" && data.kontraktor_nama) {
         allParts.push(data.kontraktor_nama);
      }
      allParts = [...new Set(allParts.filter(Boolean))];

      if (allParts.length === 3) {
          generalSalamNama = `Bapak/Ibu <strong>${data.manager_nama}</strong>, <strong>${data.pic_nama}</strong> atau <strong>${data.koordinator_nama}</strong>`;
      } else if (allParts.length > 0) {
          generalSalamNama = `Bapak/Ibu <strong>${allParts.join("</strong> atau <strong>")}</strong>`;
      } else {
          generalSalamNama = "Tim Building Support"; 
      }
  }
  ...
  let isRecipientContractor = false;
  if (recipientEmail && data.kontraktor_email) {
      isRecipientContractor = recipientEmail.toLowerCase() === data.kontraktor_email.toLowerCase();
  }

  let linkUlangHtml = '';
  if (data.link_serah_terima_ulang && (!isRejected || !isRecipientContractor)) {
      linkUlangHtml = `
  ...
  try {
    ...
    for (const recipient of recipients) {
        if (formType === "serah-terima") {
            const htmlBody = generateCommonEmailBody(
                data, 
                formType, 
                "Serah Terima", 
                "", 
             ...
             const result = await sendSingleEmail(recipient, subject, htmlBody);
             results.push({ email: recipient, messageId: result.data.id, success: true });
    ...
    return {
      ...
      message: `Email berhasil dikirim ke ${recipients.length} penerima menggunakan Gmail API`,
    };
  } catch (error) {
