  ...
  "perpanjangan_spk_approval": {
    ...
    getHtmlBody: (data) => {
        ...
        if (data.status_persetujuan === 'DITOLAK') {
            if (data.pembuat_nama) {
                namaPenerima.push(`<strong>${data.pembuat_nama}</strong>`);
            }
        } else {
        ...
        const isApproved = statusText === 'DISETUJUI' || statusText === 'DISETUJUI';
        ...
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
            ...
            <ul>
                ...
                <li><strong>Waktu Respon:</strong> ${data.waktu_persetujuan ? new Date(data.waktu_persetujuan).toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' }) : 'N/A'}</li>
    ...
    "materai_upload": {
    getSubject: (data) => `[NOTIFIKASI] Dokumen Materai Baru Telah Diunggah - ${data.ulok}`,
    getRecipients: (data) => data.recipients || [],
    getHtmlBody: (data) => {
      ...
      return `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
          ...
          <p>Email ini merupakan notifikasi bahwa dokumen materai baru telah diunggah oleh <strong>${data.uploader_name || 'N/A'}</strong> dengan rincian sebagai berikut:</p>
          <ul>
