...
function renderLinks(links, fileType) {
  if (!links || links.length === 0) {
    return `<li><strong>File ${fileType}:</strong> (Tidak ditemukan)</li>`;
  }

  if (typeof links === 'string') {
      return `<li><strong>File ${fileType}:</strong> <a href="${links}" target="_blank">${links}</a></li>`;
  }

  return links.map(link => 
    `<li><strong>File ${fileType} (${link.lingkup || 'N/A'}):</strong> <a href="${link.url}" target="_blank">${link.url}</a></li>`
  ).join('\n');
}
  ...
  return `
    <div style="text-align: left; font-family: Arial, sans-serif; line-height: 1.6;">
      ...
      <ul>
        ...
        ${renderLinks(data.spk_links, "SPK")}
        ${renderLinks(data.rab_links, "RAB & Lampiran")}
        </ul>
      ${
        data.pdfUrl
...
const emailTemplates = {
  ...
  "perpanjangan_spk_approval": {
    ...
    getHtmlBody: (data) => {
        const namaPenerima = [];
        if (data.disetujui_oleh) namaPenerima.push(`<strong>${data.disetujui_oleh}</strong>`);
        if (data.pembuat_nama) namaPenerima.push(`<strong>${data.pembuat_nama}</strong>`);   
        if (data.kontraktor_nama) namaPenerima.push(`<strong>${data.kontraktor_nama}</strong>`);

        const sapaan = namaPenerima.length > 0 ? `<p>Yth. Bapak/Ibu ${namaPenerima.join(', ')},</p>` : `<p>Pemberitahuan,</p>`;

        const statusWarna = data.status_persetujuan === 'DISETUJUI' ? 'green' : 'red';
        ...
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
            ...
            <ul>
                <li><strong>Status:</strong> <strong style="color:${statusWarna};">${data.status_persetujuan}</strong></li>
                <li><strong>Ditinjau Oleh:</strong> ${data.disetujui_oleh}</li>
                <li><strong>Waktu Respon:</strong> ${new Date(data.waktu_persetujuan).toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}</li>
                ${data.status_persetujuan === 'DITOLAK' ? `<li><strong>Alasan Penolakan:</strong> ${data.alasan_penolakan}</li>` : ''}
                <li><strong>Dokumen Terlampir:</strong> <a href="${data.link_pdf}" target="_blank">Lihat PDF</a></li>
            </ul>
            <p>Terima kasih.</p>
        </div>
        `;
