...
function generateCommonEmailBody(
  ...
  // Tentukan sapaan untuk bagian pengawasan selanjutnya
  let nextTaskSalamNama;
  if (formType === "input-pic" || ["h8", "h10", "h14", "h16", "h23", "h28", "h33", "h41"].includes(formType)) {
      // PIC yang disasar adalah Building Support
      nextTaskSalamNama = `<strong>${data.pic_nama}</strong>`;
  } else if (["h5", "h12", "h18", "h22", "h32"].includes(formType) || 
             (formType === "h25" && data.kategori_lokasi === "non_ruko_non_urugan_40hr")) {
      // Yang disasar adalah Koordinator dan Manager (kasus Ruko atau non-ruko 40hr)
      const namaParts = [data.koordinator_nama, data.manager_nama].filter(Boolean);
      nextTaskSalamNama = namaParts.length > 0 ? `<strong>${namaParts.join("</strong> dan <strong>")}</strong>` : "Tim Terkait";
  } else if (formType === "h2") {
      // h2 non-ruko: ke ketiganya. h2 ruko: ke pic.
      const isNonRuko = data.kategori_lokasi.startsWith("non_ruko");
      if (isNonRuko) {
          const namaParts = [data.pic_nama, data.koordinator_nama, data.manager_nama].filter(Boolean);
          nextTaskSalamNama = namaParts.length > 0 ? `<strong>${namaParts.join(", ")}</strong>` : "Tim Building Support";
      } else {
          nextTaskSalamNama = `<strong>${data.pic_nama}</strong>`;
      }
  } else if (formType === "h7") {
      // h7 non-ruko: ke pic. h7 ruko: ke koor dan manager.
  ...
  } else if (formType === "h25" && data.kategori_lokasi === "non_ruko_urugan_48hr") {
      // h25 non-ruko urugan 48hr: ke pic.
      nextTaskSalamNama = `<strong>${data.pic_nama}</strong>`;
  } else if (formType === "serah-terima") {
      // Serah terima, tidak ada tugas lanjut, sapaan ke semua (tetap).
  ...
  // Tentukan sapaan untuk paragraf pertama (Laporan Sebelumnya) - ke semua yang menerima
  const allRecipientsNameParts = [data.pic_nama, data.koordinator_nama, data.manager_nama].filter(Boolean);
  const generalSalamNama = allRecipientsNameParts.length > 0 ? 
    `Bapak/Ibu ${allRecipientsNameParts.map(n => `<strong>${n}</strong>`).join(", ")}` : 
    "Tim Building Support";
  ...
  if (isSupervisionSequence) {
      const previousFormType = getPreviousFormType(formType, data.kategori_lokasi);
      const nextTaskType = getNextTaskType(formType, data.kategori_lokasi);
      
      let previousReportHtml = '';
      if (data.pdfUrl) {
          previousReportHtml = `
              <p>Berikut adalah **hasil laporan ${previousFormType}** untuk toko:</p>
              <ul>
      ...
      return `
        <div style="text-align: left; font-family: Arial, sans-serif; line-height: 1.6;">
          <p>Semangat Pagi, **${generalSalamNama}**</p>
          
          ${previousReportHtml}

          <p>Yth. ${nextTaskSalamNama},</p>
          <p>Email ini merupakan surat tugas untuk **${nextTaskType}** selanjutnya terhadap proyek berikut:</p>
          <ul>
            <li>**Tanggal Pengawasan Berikutnya:** ${data.tanggal_mengawas}</li>
            ${additionalHtml}
            <li>**Kode Ulok:** ${data.kode_ulok}</li>
            <li>**Nama Toko:** ${data.nama_toko}</li>
            <li>**Cabang:** ${data.cabang}</li>
            <li>**Kategori Lokasi:** ${data.kategori_lokasi}</li>
            <li>**Tanggal Mulai SPK:** ${data.tanggal_spk}</li>
            <li>**File SPK (Sipil & ME):** <a href="${data.spkUrl}" target="_blank">${data.spkUrl}</a></li>
            <li>**File RAB & Lampiran (Sipil & ME):** <a href="${data.rabUrl}" target="_blank">${data.rabUrl}</a></li>
          </ul>
          ${
            formUrl !== "#"
  ...
  return `
    <div style="text-align: left; font-family: Arial, sans-serif; line-height: 1.6;">
      <p>Semangat Pagi,</p>
      <p>${generalSalamNama},</p>
      <p>Email ini merupakan notifikasi untuk **${taskType}** terhadap proyek berikut:</p>
      <ul>
