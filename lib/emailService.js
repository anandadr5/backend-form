...
function generateCommonEmailBody(
  ...
  const allRecipientsNameParts = [data.manager_nama, data.pic_nama, data.koordinator_nama].filter(Boolean);
  let generalSalamNama;
  if (allRecipientsNameParts.length === 3) {
  ...
  } else if (allRecipientsNameParts.length === 2) {
      generalSalamNama = `Bapak/Ibu <strong>${allRecipientsNameParts.join("</strong> dan <strong>")}</strong>`;
  } else {
  ...
  if (isSupervisionSequence) {
      ...
      if (["h8", "h14", "h16", "h23", "h28", "h33", "h41", "input-pic"].includes(formType) || 
        (formType === "h2" && data.kategori_lokasi.startsWith("ruko")) || 
        (formType === "h25" && data.kategori_lokasi === "non_ruko_urugan_48hr")
        ) 
      {
      ...
      if (nextTaskType.includes("Serah Terima") || 
          (formType === "h5" && data.kategori_lokasi === "ruko_10hr") ||
          (formType === "h12" && data.kategori_lokasi === "ruko_20hr") ||
          (formType === "h18" && data.kategori_lokasi === "non_ruko_non_urugan_30hr") ||
          (formType === "h22" && data.kategori_lokasi === "non_ruko_non_urugan_35hr") ||
          (formType === "h32" && data.kategori_lokasi === "non_ruko_urugan_48hr") 
        ) {
          const namaParts = [data.koordinator_nama, data.manager_nama].filter(Boolean);
          petugasDitugaskan = namaParts.length > 0 ? `Bapak/Ibu ${namaParts.map(n => `<strong>${n}</strong>`).join(" atau ")}` : "Tim Terkait";
      }
