...
const emailTemplates = {
  ...
  "perpanjangan_spk_approval": {
    getSubject: (data) => `Permohonan Persetujuan: Perpanjangan SPK - ${data.nomor_ulok}`,
      ...
      <div style="font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 14px; color: #333;">
        ...
        <table style="width: 100%; max-width: 600px; border-collapse: collapse; margin: 20px 0;">
          <tr style="background-color: #f2f2f2;">
            <td style="padding: 10px; border: 1px solid #ddd; width: 30%;"><strong>Nomor Ulok</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;">${data.nomor_ulok}</td>
          </tr>
          <tr>
            <td style="padding: 10px; border: 1px solid #ddd;"><strong>Diajukan Oleh</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;">${data.dibuat_oleh_nama}</td>
          </tr>
          <tr>
            <td style="padding: 10px; border: 1px solid #ddd;"><strong>Pertambahan Hari</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;">${data.pertambahan_hari} hari</td>
          </tr>
          <tr>
            <td style="padding: 10px; border: 1px solid #ddd; vertical-align: top;"><strong>Alasan</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;">${data.alasan_spk.replace(/\n/g, '<br/>')}</td>
          </tr>
          <tr>
            <td style="padding: 10px; border: 1px solid #ddd;"><strong>Dokumen Terlampir</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;"><a href="${data.pdfUrl}" target="_blank" style="color: #007bff; text-decoration: none;">Lihat Laporan PDF</a></td>
          </tr>
        </table>

        <p>Mohon untuk dapat memberikan tinjauan dan keputusan Anda dengan memilih salah satu opsi di bawah ini.</p>
        
        <table style="margin-top: 25px;" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td align="left" style="width: 150px;">
              <a href="${data.approveUrl}" style="background-color: #28a745; color: white; padding: 12px 25px; text-align: center; text-decoration: none; border-radius: 5px; display: block;">SETUJUI</a>
            </td>
            <td align="left" style="padding-left: 15px; width: 150px;">
              <a href="${data.rejectUrl}" style="background-color: #dc3545; color: white; padding: 12px 25px; text-align: center; text-decoration: none; border-radius: 5px; display: block;">TOLAK</a>
            </td>
            <td align="left"></td>
          </tr>
        </table>
        
        <p style="margin-top: 30px;">Atas perhatian dan kerja sama Bapak/Ibu, kami ucapkan terima kasih.</p>
        
        <p>Hormat kami,</p>
        <p style="margin-top: -10px;"><strong>Departemen Building & Maintenance</strong></p>
      </div>
    `
  },

    "perpanjangan_spk_notification": {
    getSubject: (data) => `Informasi: Status Permintaan Perpanjangan SPK [${data.status.toUpperCase()}] - ${data.nomor_ulok}`,
    getRecipients: (data) => data.recipients,
    getHtmlBody: (data) => `
      <div style="font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 14px; color: #333;">
        <p>Dengan hormat,</p>
        <p>Yth. Bapak/Ibu,</p>
        <p>Dengan ini kami memberitahukan bahwa permintaan perpanjangan SPK untuk <strong>Nomor Ulok ${data.nomor_ulok}</strong> telah selesai diproses dengan hasil sebagai berikut:</p>
        
        <table style="width: 100%; max-width: 600px; border-collapse: collapse; margin: 20px 0;">
          <tr style="background-color: #f2f2f2;">
            <td style="padding: 10px; border: 1px solid #ddd; width: 30%;"><strong>Status Final</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;">
              <strong style="color:${data.status === 'disetujui' ? '#28a745' : '#dc3545'};">${data.status.toUpperCase()}</strong>
            </td>
          </tr>
          <tr>
            <td style="padding: 10px; border: 1px solid #ddd;"><strong>Ditinjau Oleh</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;">${data.disetujui_oleh}</td>
          </tr>
          <tr>
            <td style="padding: 10px; border: 1px solid #ddd;"><strong>Waktu Keputusan</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;">${new Date(data.waktu_persetujuan).toLocaleString('id-ID', { dateStyle: 'full', timeStyle: 'long' })}</td>
          </tr>
          ${data.status === 'ditolak' ? `
          <tr>
            <td style="padding: 10px; border: 1px solid #ddd; vertical-align: top;"><strong>Alasan Penolakan</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;">${data.alasan_penolakan || 'Tidak ada alasan yang diberikan.'}</td>
          </tr>` : ''}
          <tr>
            <td style="padding: 10px; border: 1px solid #ddd;"><strong>Laporan Final</strong></td>
            <td style="padding: 10px; border: 1px solid #ddd;"><a href="${data.link_pdf}" target="_blank" style="color: #007bff; text-decoration: none;">Lihat Dokumen PDF</a></td>
          </tr>
        </table>
        
        <p>Informasi ini telah tercatat dalam sistem. Terima kasih atas perhatiannya.</p>
        
        <p style="margin-top: 30px;">Hormat kami,</p>
        <p style="margin-top: -10px;"><strong>Sistem Notifikasi Otomatis</strong></p>
      </div>
    `
  }
};
