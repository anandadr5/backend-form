  ...
  "serah-terima": {
    ...
    getRecipients: (data) => {
      ...
      if (isRejected) {
        ...
        if (data.koordinator_email) recipients.push(data.koordinator_email);
        if (data.manager_email) recipients.push(data.manager_email);
        if (data.kontraktor_email) recipients.push(data.kontraktor_email);
      }
    ...
    if (formType === "serah-terima") {
      ...
      if (isRejected) {
        ...
        if (recipients1.length > 0) {
          const subject1 = template1.getSubject(data);
          const htmlBody1 = template1.getHtmlBody(data);
          const rawMessage1 = createEmailMessage(recipients1, subject1, htmlBody1);
          const encodedMessage1 = Buffer.from(rawMessage1)
            .toString("base64")
            .replace(/\+/g, "-")
            .replace(/\//g, "_")
            .replace(/=+$/, "");
          ...
          console.log("✅ Email pertama terkirim ke Manager, PIC, Koordinator");
        }

        if (data.kontraktor_email) {
          ...
          if (recipients2.length > 0) {
            const subject2 = template2.getSubject(data);
            const htmlBody2 = template2.getHtmlBody(data);
            const rawMessage2 = createEmailMessage(recipients2, subject2, htmlBody2);
            const encodedMessage2 = Buffer.from(rawMessage2)
              .toString("base64")
              .replace(/\+/g, "-")
              .replace(/\//g, "_")
              .replace(/=+$/, "");
            ...
            console.log("✅ Email kedua terkirim ke Kontraktor");
          }
        }

        return {
          success: true,
          message: "Email berhasil dikirim (2 email untuk status TIDAK DITERIMA)",
          recipients: [...recipients1, data.kontraktor_email].filter(Boolean),
        };
      }
    }
