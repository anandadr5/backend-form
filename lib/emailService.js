...
function generateCommonEmailBody(
  ...
  } else if (formType === "h10" && data.kategori_lokasi === "ruko_14hr") {
    salamNama = `<strong>${data.pic_nama}</strong>`;
  } else if (
    ["h5", "h7", "h12", "h18", "h22", "h25", "h32"].includes(formType) ||
    (formType === "h10" && data.kategori_lokasi !== "ruko_14hr")
  ) {
...
// Fungsi untuk mengirim email (tidak berubah)
