...
function getMissingGmailEnv() {
  return requiredGmailEnv.filter((key) => !process.env[key]);
}

function getGmailErrorInfo(error) {
  const responseError = error.response?.data?.error;
  const responseDescription = error.response?.data?.error_description;
  const errorCode =
    typeof responseError === "string" ? responseError : responseError?.code;
  const errorMessage =
    typeof responseError === "string"
      ? responseDescription || responseError
      : responseError?.message || responseDescription || error.message;
  ...
  if (errorCode === "invalid_grant" || errorMessage?.includes("invalid_grant")) {
    return {
      ...
      error: "invalid_grant",
      message:
        "Token Gmail sudah tidak valid atau dicabut. Buat ulang GOOGLE_REFRESH_TOKEN dengan akun Gmail pengirim yang benar, lalu update environment variable di Render.",
      actionRequired:
        "Regenerate OAuth refresh token untuk Gmail API dan redeploy backend.",
    };
  }
...
function asEmailList(value) {
  if (!value) return [];
  if (Array.isArray(value)) return value.flatMap(asEmailList);
...
function uniqueEmailList(...values) {
  ...
  for (const email of values.flatMap(asEmailList)) {
...
async function sendGmailMessage(recipients, subject, htmlBody) {
  const finalRecipients = uniqueEmailList(recipients);
  if (finalRecipients.length === 0) return null;

  const rawMessage = createEmailMessage(finalRecipients, subject, htmlBody);
...
function generateCommonEmailBody(
  ...
  const opnameLinkHtml =
    formType === "input-pic"
      ? `<p>Silakan lakukan opname melalui link berikut: <a href="https://sparta-alfamart.vercel.app/" target="_blank">https://sparta-alfamart.vercel.app/</a></p>`
      : "";
  ...
  return `
    <div style="text-align: left; font-family: Arial, sans-serif; line-height: 1.6;">
      ...
      <ul>
        ...
        <li><strong>Kategori Lokasi:</strong> ${data.kategori_lokasi}</li>
        <li><strong>Tanggal Mulai SPK:</strong> ${data.tanggal_spk}</li>
        <li><strong>File SPK (Sipil & ME):</strong> <a href="${data.spkUrl
    }" target="_blank">${data.spkUrl}</a></li>
        <li><strong>File RAB & Lampiran (Sipil & ME):</strong> <a href="${data.rabUrl
...
const emailTemplates = {
  ...
  "serah-terima": {
    ...
    getHtmlBody: (data) => {
      ...
      if (isRejected) {
        ...
        generalSalamNameParts = [data.manager_nama, data.pic_nama, data.koordinator_nama, data.kontraktor_nama].filter(Boolean);
      }

      let generalSalamNama;
      if (generalSalamNameParts.length === 4) {
        generalSalamNama = `Bapak/Ibu <strong>${data.manager_nama}</strong>, <strong>${data.pic_nama}</strong>, dan <strong>${data.koordinator_nama}</strong> atau <strong>${data.kontraktor_nama}</strong>`;
      } else if (generalSalamNameParts.length === 3) {
        generalSalamNama = `Bapak/Ibu <strong>${generalSalamNameParts[0]}</strong> dan <strong>${generalSalamNameParts[1]}</strong> atau <strong>${generalSalamNameParts[2]}</strong>`;
      } else if (generalSalamNameParts.length > 0) {
        generalSalamNama = `Bapak/Ibu <strong>${generalSalamNameParts.join("</strong> atau <strong>")}</strong>`;
      } else {
        generalSalamNama = "Tim Building Support";
      ...
      if (isRejected && data.tanggal_serah) {
        statusListItems += `<li><strong>Tanggal Serah Terima Berikutnya:</strong> ${data.tanggal_serah}</li>`;
      } else if (!isRejected) {
        statusListItems += `<li><strong>Tanggal Serah Terima:</strong> ${data.tanggal_mengawas}</li>`;
      }
      ...
      <div style="text-align: left; font-family: Arial, sans-serif; line-height: 1.6;">
        <p>Semangat Pagi,</p>
        <p>Yth. ${generalSalamNama},</p>
        <p>Email ini merupakan notifikasi untuk <strong>Serah Terima</strong> terhadap proyek berikut:</p>
        <ul>
          ${statusListItems}
          <li><strong>Kode Ulok:</strong> ${data.kode_ulok}</li>
          <li><strong>Nama Toko:</strong> ${data.nama_toko}</li>
        ...
        ${data.pdfUrl ? `<p>📄 <a href="${data.pdfUrl}" target="_blank">Lihat Laporan PDF</a></p>` : ""}
        
        ${(formUrl !== "#" && !isRejected) ? `<p>Mohon untuk segera mengisi laporan **Serah Terima** Anda melalui link di bawah ini:</p><p>📌 <a href="${formUrl}" target="_blank">Isi Laporan Serah Terima</a></p>` : ""}
  ...
  "serah-terima-kontraktor": {
    getSubject: (data) =>
      `Informasi Serah Terima untuk Toko: ${data.kode_ulok}`,
    getRecipients: (data) => {
      return data.kontraktor_email ? [data.kontraktor_email] : [];
    },
    getHtmlBody: (data) => {
      const isRejected = (data.status_serah || '').toUpperCase() === 'TIDAK DITERIMA';
      const generalSalamNama = data.kontraktor_nama
        ? `Bapak/Ibu <strong>${data.kontraktor_nama}</strong>`
        : "Tim Kontraktor";

      const statusListItems = `
      <li><strong>Status Serah Terima:</strong> <strong>${data.status_serah || 'N/A'}</strong></li>
      ${data.tanggal_serah ? `<li><strong>Tanggal Serah Terima Berikutnya:</strong> ${data.tanggal_serah}</li>` : ''}
    `;
      ...
      <div style="text-align: left; font-family: Arial, sans-serif; line-height: 1.6;">
        ...
        ${isRejected
        ? `<p><strong>PENTING:</strong> Serah Terima ini <strong>TIDAK DITERIMA</strong>. Mohon segera melakukan perbaikan sesuai catatan yang diberikan.</p>`
        : `<p>Serah Terima ini telah <strong>DITERIMA</strong>. Mohon arsipkan informasi ini sesuai kebutuhan.</p>`
      }
    ...
    getHtmlBody: (data) => `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
          ...
          <ul>
            ...
            ${data.userFileUrl && data.userFileUrl !== '-' ? `<li><strong>Lampiran Pendukung:</strong> <a href="${data.userFileUrl}" target="_blank">Lihat Lampiran</a></li>` : ''}
          </ul>
  ...
  "perpanjangan_spk_notification": {
    ...
    getHtmlBody: (data) => {
      ...
      let lampiranUserHtml = '';
      if (data.link_lampiran_user && data.link_lampiran_user !== '-' && data.link_lampiran_user.trim() !== '') {
        lampiranUserHtml = `<li><strong>Lampiran Pendukung:</strong> <a href="${data.link_lampiran_user}" target="_blank">Lihat Lampiran</a></li>`;
      }
...
async function sendEmail(formType, data) {
  try {
    const missingEnv = getMissingGmailEnv();
    if (missingEnv.length > 0) {
      return {
        success: false,
        code: "GMAIL_ENV_MISSING",
        error: `Environment variable belum lengkap: ${missingEnv.join(", ")}`,
        message:
          "Konfigurasi Gmail API di backend belum lengkap. Lengkapi environment variable lalu redeploy backend.",
        missingEnv,
      };
    }

    if (formType === "serah-terima") {
      const isRejected = (data.status_serah || '').toUpperCase() === 'TIDAK DITERIMA';
      const sentRecipients = [];

      const template1 = emailTemplates["serah-terima"];
      const recipients1 = template1.getRecipients(data);

      if (recipients1.length > 0) {
        await sendGmailMessage(
          recipients1,
          template1.getSubject(data),
          template1.getHtmlBody(data)
        );
        sentRecipients.push(...recipients1);
        console.log("✅ Email serah terima terkirim ke Manager, PIC, Koordinator");
      }

      const template2 = emailTemplates["serah-terima-kontraktor"];
      const recipients2 = template2.getRecipients(data);

      if (recipients2.length > 0) {
        await sendGmailMessage(
          recipients2,
          template2.getSubject(data),
          template2.getHtmlBody(data)
        );
        sentRecipients.push(...recipients2);
        console.log("✅ Email serah terima terkirim ke Kontraktor");
      }

      return {
        success: true,
        message: isRejected
          ? "Email serah terima TIDAK DITERIMA berhasil dikirim terpisah"
          : "Email serah terima DITERIMA berhasil dikirim terpisah",
        recipients: uniqueEmailList(sentRecipients),
      };
    }

    if (formType === "materai_upload") {
      const managerRecipients = data.managerRecipients || [];
      const otherRecipients = data.otherRecipients || [];
      const allSentRecipients = [];

      if (managerRecipients.length > 0) {
        const template = emailTemplates["materai_upload_manager"];
        const subject = template.getSubject(data);
        const htmlBody = template.getHtmlBody({
          ...data,
          recipients: managerRecipients.map(r => r.email),
          recipientNames: managerRecipients.map(r => r.name),
        });
        const rawMessage = createEmailMessage(managerRecipients.map(r => r.email), subject, htmlBody);
        const encodedMessage = Buffer.from(rawMessage)
          .toString("base64")
          .replace(/\+/g, "-")
          .replace(/\//g, "_")
        ...
        allSentRecipients.push(...managerRecipients.map(r => r.email));
        console.log(`✅ Email Manager terkirim ke: ${managerRecipients.map(r => r.email).join(', ')}`);
      }

      if (otherRecipients.length > 0) {
        const template = emailTemplates["materai_upload_coordinator"];
        const subject = template.getSubject(data);
        const htmlBody = template.getHtmlBody({
          ...data,
          recipients: otherRecipients.map(r => r.email),
          recipientNames: otherRecipients.map(r => r.name),
        });
        const rawMessage = createEmailMessage(otherRecipients.map(r => r.email), subject, htmlBody);
        const encodedMessage = Buffer.from(rawMessage)
          .toString("base64")
          .replace(/\+/g, "-")
          .replace(/\//g, "_")
        ...
        allSentRecipients.push(...otherRecipients.map(r => r.email));
        console.log(`✅ Email Koordinator terkirim ke: ${otherRecipients.map(r => r.email).join(', ')}`);
      }

      if (allSentRecipients.length === 0) {
        throw new Error("Tidak ada penerima email yang ditemukan");
      }

      return {
        success: true,
        message: `Email berhasil dikirim ke ${allSentRecipients.length} penerima`,
        recipients: allSentRecipients,
      };
    }
...
async function verifyConnection() {
  try {
    ...
    return {
      success: false,
      error: error.message,
      message: "Gagal memverifikasi koneksi Gmail API",
    };
  }
}
