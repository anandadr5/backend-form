...
const emailTemplates = {
  ...
  "perpanjangan_spk_approval": {
    ...
    "materai_upload": {
    getSubject: (data) => `Dokumen Final Termaterai (RAB & SPH) - ${data.ulok}`,
    getRecipients: (data) => data.recipients || [],
    getHtmlBody: (data) => {
      ...
      const rabLink = data.rabUrl || '#';
      const sphLink = data.sphUrl || '#';
      
      return `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
          <p>Semangat Pagi,</p>
          <p>Bapak/Ibu ${sapaanNama},</p>
          <p>Email ini merupakan notifikasi bahwa dokumen materai **RAB dan SPH** telah diunggah dan disimpan dengan rincian sebagai berikut:</p>
          <ul>
          ...
          <p>
            📄 <strong>Rekap RAB:</strong> <a href="${rabLink}" target="_blank">Lihat Dokumen RAB PDF</a>
            <br/>
            📄 <strong>SPH:</strong> <a href="${sphLink}" target="_blank">Lihat Dokumen SPH PDF</a>
          </p>
