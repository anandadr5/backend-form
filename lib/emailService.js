...
function generateCommonEmailBody(
  ...
  if (formType === "input-pic") {
    ...
    ["h2", "h7", "h8", "h10", "h14", "h16", "h17", "h23", "h25", "h28", "h33", "h41"].includes(formType)
  ) {
    const namaParts = [data.pic_nama];
    if (data.koordinator_nama) namaParts.push(data.koordinator_nama);
    if (data.manager_nama) namaParts.push(data.manager_nama);
    salamNama = `<strong>${namaParts.filter(Boolean).join(", ")}</strong>`;
  ...
  if (isSupervisionSequence) {
      ...
      return `
        <div style="text-align: left; font-family: Arial, sans-serif; line-height: 1.6;">
          ...
          <p>Yth. Bapak/Ibu ${salamNama},</p>
          <p>Email ini merupakan surat tugas untuk ${taskType} selanjutnya terhadap proyek berikut:</p>
          <ul>
          ...
          ${
            formUrl !== "#"
              ? `<p>Mohon untuk segera mengisi laporan ${isSerahTerima ? "Serah Terima" : "pengawasan"} Anda melalui link di bawah ini:</p>
