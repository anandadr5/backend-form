...
  const isSerahTerimaForm = formType === "serah-terima";
  
...
  } else if (isSerahTerimaForm) {
      if (data.isContractorOnlyNotification) { 
          generalSalamNameParts = [data.kontraktor_nama].filter(Boolean);
      } else if (isRejected) { 
...
  if (isSerahTerimaForm && data.isContractorOnlyNotification && generalSalamNameParts.length > 0) {
      generalSalamNama = `Bapak/Ibu <strong>${data.kontraktor_nama}</strong>`;
  } else if (isSerahTerimaForm && !isRejected && generalSalamNameParts.length >= 4) {
...
  }
  ...
  const isRejectedFinal = (data.status_serah || '').toUpperCase() === 'TIDAK DITERIMA';
  
  let statusListItems = `<li><strong>Status Serah Terima:</strong> <strong>${data.status_serah || 'N/A'}</strong></li>`;
  
  if (isRejectedFinal && additionalInfo.tglSerahBerikut) {
...
  let linkUlangHtml = '';
  if (data.link_serah_terima_ulang && !data.isContractorOnlyNotification) {
      linkUlangHtml = `
...
const emailTemplates = {
  ...
  "serah-terima": {
    ...
    getRecipients: (data) => {
      const recipients = data.recipientsOverride || [];
      
      if (recipients.length > 0) return recipients.filter(Boolean);
