...
function generateCommonEmailBody(
  ...
  if (isSupervisionSequence) {
      ...
      if (data.pdfUrl) {
          previousReportHtml = `
              <p>Berikut adalah <strong>hasil laporan ${previousFormType}</strong> untuk toko:</p>
              <ul>
      ...
      return `
        <div style="text-align: left; font-family: Arial, sans-serif; line-height: 1.6;">
          <p>Semangat Pagi, <strong>${generalSalamNama}</strong></p>
          
          ${previousReportHtml}

          <p>Yth. ${nextTaskSalamNama},</p>
          <p>Email ini merupakan surat tugas untuk <strong>${nextTaskType}</strong> selanjutnya terhadap proyek berikut:</p>
          <ul>
